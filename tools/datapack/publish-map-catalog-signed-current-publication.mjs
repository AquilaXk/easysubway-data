...
async function exactEntries(root, expected, label) { const actual = (await readdir(root)).sort(); const wanted = [...expected].sort(); if (canonicalJson(actual) !== canonicalJson(wanted)) throw new Error(`${label} entries mismatch`); }
