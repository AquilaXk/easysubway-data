...
export function syncCurrentRouteEdgePolicy(input, policy) {
  ...
  next.rideInvariant.itxCheongchunExpress.admittedEdgeSetSha256 = canonicalRideEdgeSetSha256(itx);
  return next;
}

export async function syncCurrentRouteEdgePolicyFile({ inputPath, policyPath, outputPath = policyPath }) {
  ...
  const next = syncCurrentRouteEdgePolicy(JSON.parse(inputBytes), JSON.parse(policyBytes));
