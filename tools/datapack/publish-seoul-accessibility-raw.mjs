...
export async function publishSeoulAccessibilityRawArtifact({
  ...
  const snapshot = validateSeoulAccessibilitySnapshotIdentity(JSON.parse(snapshotBytes));
  const rawArtifact = validateSeoulAccessibilityRawCollection(JSON.parse(rawArtifactBytes), snapshot);
  ...
  const { head, trustedBucketOwner, idempotentExistingObject } = await publishImmutableKricRawObject({
    ...
    rawObjectSha256,
    artifactKind: RAW_ARTIFACT_KIND,
    sourceId: SOURCE_ID,
  });
  const receipt = {
    schemaVersion: 1,
...
function validateManifest(value) {
  ...
  if (!exactKeys(value, keys)
    ...
    || value.artifactKind !== OBSERVATION_ARTIFACT_KIND
    ...
    throw new Error("Seoul accessibility observation manifest is invalid");
  }
}
...
function validateObservationIdentity({ manifest, snapshot, rawArtifact, snapshotBytes, rawArtifactBytes }) {
  if (manifest.sourceId !== snapshot.sourceId
    ...
    throw new Error("Seoul accessibility observation identity mismatch");
  }
}

function containedFile(root, filename) {
  if (typeof filename !== "string" || filename === "" || path.basename(filename) !== filename) {
    throw new Error("Seoul accessibility observation path is invalid");
  }
  const resolved = path.resolve(root, filename);
  if (path.dirname(resolved) !== root) throw new Error("Seoul accessibility observation path is invalid");
  return resolved;
}
