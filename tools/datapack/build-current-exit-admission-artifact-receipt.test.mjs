...
test("213/199/420 synthetic admission ZIP을 canonical 3-file receipt로 deterministically publish한다", async () => {
  ...
  try {
    ...
    assert.notEqual(admission.sourceIdentity.facilityStationLineMappingSha256, admission.stationLineMappingSha256);
    ...
    const malformedExitMapping = structuredClone(admission); malformedExitMapping.stationLineMappingSha256 = "f".repeat(64); rehashAdmission(malformedExitMapping); const malformedExitMappingArchive = zip([{ name: normalizedName, bytes: normalizedBytes }, { name: admissionName, bytes: Buffer.from(canonical(malformedExitMapping)) }]);
    assert.throws(() => buildCurrentExitAdmissionArtifactReceipt({ ...input, artifactArchiveBytes: malformedExitMappingArchive, artifactArchiveSha256: sha256(malformedExitMappingArchive) }), /admission cell binding mismatch/);
    const malformedFacilityMapping = structuredClone(admission); malformedFacilityMapping.sourceIdentity.facilityStationLineMappingSha256 = "not-a-sha"; rehashAdmission(malformedFacilityMapping); const malformedFacilityMappingArchive = zip([{ name: normalizedName, bytes: normalizedBytes }, { name: admissionName, bytes: Buffer.from(canonical(malformedFacilityMapping)) }]);
    assert.throws(() => buildCurrentExitAdmissionArtifactReceipt({ ...input, artifactArchiveBytes: malformedFacilityMappingArchive, artifactArchiveSha256: sha256(malformedFacilityMappingArchive) }), /admission source hash mismatch/);
