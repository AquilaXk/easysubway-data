...
function validateIdentity(value, label, now) {
  if (!GIT_SHA.test(value.producerGitSha ?? "") || !Number.isSafeInteger(value.releaseSequence) || value.releaseSequence < 1 || !SHA.test(value.signedFinalDescriptorSha256 ?? "") || !SHA.test(value.stationSetSha256 ?? "") || !Number.isFinite(Date.parse(value.freshUntil)) || Date.parse(value.freshUntil) <= now || value.keyId !== signingKeyId()) throw new Error(`${label} identity mismatch`);
}
...
function selfDigest(value, field, label) {
  if (!SHA.test(value[field] ?? "")) throw new Error(`${label} digest malformed`);
