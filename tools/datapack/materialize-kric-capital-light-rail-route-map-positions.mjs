...
const VERIFICATION_TEST = "tools/datapack/materialize-kric-capital-light-rail-route-map-positions.test.mjs";

export function materializeCapitalLightRailRouteMapPositions({
  baseFixture,
  snapshot,
  snapshotSha256,
  topologySnapshot,
  ...
  if (!pack || fixture.packs.length !== 1 || pack.artifactKind !== "production") {
    throw new Error("capital-light rail route map positions require one cumulative production pack");
  }
  ...
  const composition = sha256(JSON.stringify({
    ...
    source,
    contentSha256: materializedCapitalLightRailRouteMapPackContentHash(pack, version),
  }));
...
export function materializedCapitalLightRailRouteMapPackContentHash(pack, version) {
  const content = { ...pack };
  delete content.id;
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  try {
    ...
    console.error(error instanceof Error ? error.message : "capital-light rail route map position materialization failed");
    process.exitCode = 1;
  }
}
