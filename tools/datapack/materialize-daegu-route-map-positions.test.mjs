...
const routeMapNow = new Date("2026-07-24T03:00:00.000Z");
...
async function inputs() {
  const [
    base, busanTopology, busanTimetable,
...
test("공식 대구 출구 위경도 snapshot을 누적 production candidate pack에 materialize한다", async () => {
  const { baseFixture, daeguSnapshot, daeguSnapshotSha256, topologySnapshots, inventory } = await inputs();
  const fixture = materializeDaeguRouteMapPositions({
    baseFixture,
    snapshot: daeguSnapshot,
    snapshotSha256: daeguSnapshotSha256,
    topologySnapshots,
    inventory,
    now: routeMapNow,
  });
  ...
  assert.match(pack.id, /^nationwide-daegu-route-map-[a-f0-9]{64}$/);
  ...
  assert.throws(
    () => materializeDaeguRouteMapPositions({
      baseFixture, snapshot: daeguSnapshot, snapshotSha256: daeguSnapshotSha256,
      topologySnapshots, inventory: mismatchedInventory, now: routeMapNow,
    }),
    /inventory evidence/,
  );
  const byteDifferentSnapshotSha256 = createHash("sha256")
    .update(`${JSON.stringify(daeguSnapshot, null, 2)}\n`)
    .digest("hex");
  assert.notEqual(byteDifferentSnapshotSha256, daeguSnapshotSha256);
  assert.throws(
    () => materializeDaeguRouteMapPositions({
      baseFixture, snapshot: daeguSnapshot, snapshotSha256: byteDifferentSnapshotSha256,
      topologySnapshots, inventory, now: routeMapNow,
    }),
    /snapshot byte identity/,
  );
});

test("materialized SQLite와 provenance가 대구 1·2·3호선 route_map_positions를 SUPPORTED로 만든다", async (context) => {
  const outputDir = await mkdtemp(path.join(tmpdir(), "easysubway-daegu-route-map-pack-"));
  ...
  const routeMapRequirements = report.requirements.filter(
    ({ operatorId, sourceDomain }) => operatorId === "daegu-transportation" && sourceDomain === "route_map_positions",
  );
  assert.equal(routeMapRequirements.length, 3);
