...
import { buildCurrentCapitalLiveChainBundle, readCurrentCapitalLiveChainBundle } from "./build-current-capital-live-chain-bundle.mjs";
...
test("composite bundle embeds canonical fan-in evidence without expanding the output allowlist", async () => {
  ...
  const outputPaths = [...Object.values(CURRENT_CAPITAL_LIVE_CHAIN_FAN_IN_COMPONENT_PATHS), "provider-receipt.json"];
  ...
  const options = { root, outputDirectory: path.join(root, "out"), repository: "AquilaXk/easysubway-data", repositorySha: "b".repeat(40), operationId: "current-capital-560", providerReceiptRelativePath: "provider-receipt.json", outputPaths, boundaryBytes: boundaryFor(entryBytes) };
  const bytes = await buildCurrentCapitalLiveChainBundle(options);
  const readOptions = omit(options, "root", "outputDirectory", "boundaryBytes", "providerReceiptRelativePath");
  const bundle = readCurrentCapitalLiveChainBundle(bytes, readOptions);
  assert.equal(bundle.entries.length, outputPaths.length);
