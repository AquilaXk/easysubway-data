...
export async function buildMapCatalogSignedCurrentPublication({ artifactRoot, output, producerGitSha, releaseSequence, signedFinalDescriptorSha256, freshUntil, privateKey = signingPrivateKey(), publicKey, now = Date.now() } = {}) {
  ...
  validateMapCatalogSignedCurrentPublication(descriptor, { publicKey });
...
async function readPack(root, name, paths) { const pack = await directory(path.join(root, name), `${name} root`); await entries(pack, ["manifest.json", "payload"], `${name} root`); await entries(path.join(pack, "payload"), paths.filter((item) => item.startsWith("payload/")).map((item) => item.slice(8)), `${name} payload`); const objects = []; for (const relative of paths) { const bytes = await regular(pack, relative, `${name} ${relative}`); objects.push({ path: relative, sizeBytes: bytes.length, sha256: sha(bytes) }); } const manifestBytes = objects[0] && await regular(pack, "manifest.json", `${name} manifest`); let manifest; try { manifest = JSON.parse(manifestBytes.toString("utf8")); } catch { throw new Error(`${name} manifest must be JSON`); } if (!Buffer.from(canonicalJson(manifest)).equals(manifestBytes)) throw new Error(`${name} manifest is noncanonical`); return { manifest, objects }; }
...
async function entries(root, expected, label) { const actual = (await readdir(root)).sort(); const wanted = [...expected].sort(); if (JSON.stringify(actual) !== JSON.stringify(wanted)) throw new Error(`${label} entries mismatch`); }
