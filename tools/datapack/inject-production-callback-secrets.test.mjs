...
const script = new URL("tools/datapack/inject-production-callback-secrets.mjs", root);
...
test("rejects invalid secrets before changing the dotenv", () => {
  const cases = [
    ...
    [{ [tokenKey]: "workflow-token-value\u00a0" }, tokenKey],
    [{ [tokenKey]: "line\nbreak" }, tokenKey],
    [{ [tokenKey]: "workflow#token" }, tokenKey],
    [{ [tokenKey]: "workflow'token" }, tokenKey],
    [{ [tokenKey]: 'workflow"token' }, tokenKey],
    [{ [tokenKey]: "workflow`token" }, tokenKey],
    ...
    [{ [hmacKey]: `${hmac} ` }, hmacKey],
    [{ [hmacKey]: "callback#hmac-key-with-at-least-32-bytes" }, hmacKey],
    [{ [hmacKey]: "callback'hmac-key-with-at-least-32-bytes" }, hmacKey],
    [{ [hmacKey]: 'callback"hmac-key-with-at-least-32-bytes' }, hmacKey],
    [{ [hmacKey]: "callback`hmac-key-with-at-least-32-bytes" }, hmacKey],
  ];
...
function makeFixture(contents) {
  ...
  const path = join(directory, "easysubway-deploy.env");
