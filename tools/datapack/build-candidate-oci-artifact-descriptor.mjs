...
function exactKeys(value, keys, label) { if (!value || typeof value !== "object" || Array.isArray(value) || Object.keys(value).sort(compareUtf8).join("\0") !== [...keys].sort(compareUtf8).join("\0")) throw new Error(`${label} fields must be exact`); }
