...
test("registers exactly six reviewed outputs and preserves atomic prestate", async (t) => {
  ...
  assert.equal(source.requiredForProductionPack, true); assert.equal(source.accessibilityAdmissionEvidence.issue, 622); assert.equal(Object.hasOwn(source.accessibilityAdmissionEvidence, "adminReviewRecordHash"), false); assert.equal(Object.hasOwn(source.accessibilityAdmissionEvidence, "rawReceipt"), false); assert.equal(Object.hasOwn(source.accessibilityAdmissionEvidence, "claimBindingsSha256"), false); assert.deepEqual(source.admissionEvidence, originalSource.admissionEvidence);
