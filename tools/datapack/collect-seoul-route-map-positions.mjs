...
export function buildSeoulRouteMapPositions({ records, topologySnapshotBytes, topologySnapshotId, now = new Date(), rawSha256 } = {}) {
  ...
  const layoutPositions = layout(rawPositions, order, joinLedger); const layoutTracks = tracks(rawPositions, layoutPositions, order, joinLedger); const legacy = legacyAdmission(rawPositions); const scope = rawPositions.map(scopeRow); const lineOrderSha256 = hash(json(order)); const aliasLedgerSha256 = hash(json(aliases)); const outputSchemaSha256 = hash(json(schema()));
  const snapshot = {
...
export function validateSeoulRouteMapPositionsSnapshot(snapshot, { topologySnapshotBytes } = {}) {
  try {
    ...
    const joinLedger = buildJoinLedger(snapshot.rawPositions, order); const layoutPositions = layout(snapshot.rawPositions, order, joinLedger); const layoutTracks = tracks(snapshot.rawPositions, layoutPositions, order, joinLedger); const legacy = legacyAdmission(snapshot.rawPositions); const scope = snapshot.rawPositions.map(scopeRow);
    const legacyDiagnostic = { stationCount: legacy.positions.length, lineStationCounts: counts(legacy.positions), positions: legacy.positions, positionsSha256: hash(JSON.stringify(legacy.positions)), quarantinedCount: legacy.quarantinedPositions.length, quarantinedPositions: legacy.quarantinedPositions };
    if (!same(snapshot.layoutPositions, layoutPositions) || !same(snapshot.layoutTracks, layoutTracks) || snapshot.layoutPositionsSha256 !== hash(json(layoutPositions)) || snapshot.layoutTracksSha256 !== hash(json(layoutTracks)) || snapshot.outputSchemaSha256 !== hash(json(schema())) || snapshot.semanticInputSha256 !== semanticInput(snapshot) || snapshot.semanticOutputSha256 !== semanticOutput(snapshot) || snapshot.stationCount !== layoutPositions.length || !same(snapshot.lineStationCounts, counts(snapshot.rawPositions)) || !same(snapshot.legacyDiagnostic, legacyDiagnostic) || !same(snapshot.scope, scope) || snapshot.scopeSha256 !== hash(json(scope))) throw Error("derived");
...
function normalize(records) {
  ...
  const rawPositions = records.map((r, i) => {
    ...
    const lat = micro(r?.latitude, "latitude", key); const lon = micro(r?.longitude, "longitude", key); if (lat < CANVAS.minLat || lat > CANVAS.maxLat || lon < CANVAS.minLon || lon > CANVAS.maxLon) throw new Error(`Seoul route map positions invalid coordinates: ${key}`);
    ...
    return { line, lineId, stationCode, stationName, latitude: lat / 1_000_000, longitude: lon / 1_000_000, basisDate };
...
function legacyAdmission(rawPositions) { const groups = new Map(); for (const raw of rawPositions) { const g = groups.get(`${raw.latitude},${raw.longitude}`) ?? []; g.push(raw); groups.set(`${raw.latitude},${raw.longitude}`, g); } const positions = [], quarantinedPositions = []; for (const g of groups.values()) if (new Set(g.map((x) => x.stationName)).size <= 1) positions.push(...g.map(withStationId)); else for (const raw of g) quarantinedPositions.push({ ...withStationId(raw), reasonCode: "OFFICIAL_DUPLICATE_LATLON" }); return { positions: positions.sort(comparePosition), quarantinedPositions: quarantinedPositions.sort(comparePosition) }; }
