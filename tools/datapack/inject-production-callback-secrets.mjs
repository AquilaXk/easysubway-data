...
function requiredSecret(environment, key, { minimumUtf8Bytes = 1 } = {}) {
  ...
  if (/[#'"`]/.test(value)) fail(`${key} contains dotenv-significant characters`);
  return value;
}
...
function deploymentEnvPath(environment) {
  ...
  return join(runnerTemp, "easysubway-deploy.env");
}
...
export function inject(environment = process.env) {
  const values = {
    workflowToken: requiredSecret(environment, workflowTokenKey),
...
if (import.meta.url === new URL(process.argv[1], "file:").href) {
  try {
    if (process.argv.length !== 2) fail("usage: inject-production-callback-secrets.mjs");
    inject();
  } catch (error) {
