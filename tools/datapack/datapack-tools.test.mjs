...
test("데이터팩 생성기는 production pack의 최소 row 기준 누락을 거부한다", async () => {
  ...
  await assert.rejects(
    ...
    /production minimumTableRows must define stations, station_lines, network_edges, and facilities/,
  );
});
...
test("데이터팩 검증기는 production manifest의 최소 row 기준 누락을 거부한다", async () => {
  ...
  await assert.rejects(
    ...
    /capital@1 production minimumTableRows must define stations, station_lines, network_edges, and facilities/,
  );
});
