...
function parseArgs(argv) { if (!Array.isArray(argv) || argv.length !== 6 || argv[0] !== "--canonical-pack" || argv[2] !== "--transfer-topology-metrics" || argv[4] !== "--output" || !argv.slice(1).filter((_, index) => index % 2 === 0).every(path.isAbsolute)) throw new Error("arguments must be --canonical-pack <absolute> --transfer-topology-metrics <absolute> --output <absolute absent>"); return { canonicalPack: path.resolve(argv[1]), transferTopologyMetrics: path.resolve(argv[3]), output: path.resolve(argv[5]) }; }
