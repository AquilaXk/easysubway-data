...
test("topology refresh workflow is a pinned, main-only, durable claim automation", () => {
  ...
  assert.match(yml, /registrationEvidence\.snapshotId/); assert.match(yml, /itxTopologyEvidencePath/); assert.match(yml, /source_key="\$\{key\}_SOURCE"; source="\$\{!source_key\}"/); assert.match(yml, /itxRefreshRequired: process\.env\.ITX_REFRESH_REQUIRED === "true"/); assert.match(yml, /exactly eleven or twelve paths/); assert.match(yml, /four-input topology claim must not select an ITX admission/); assert.match(yml, /five-input topology claim ITX admission binding is invalid/); assert.match(yml, /exactly four or five immutable current topology inputs/); assert.match(yml, /exactly seven activation paths/);
  assert.equal((yml.match(/itx_args=\(\)/g) ?? []).length, 2);
