#!/usr/bin/env bash
# #2011 3단계: 광주 오너 자작 도식(easy-subway-gwangju-v*) 반복 파이프라인 — 한 줄 재실행.
#
# 부산 run-sma-pipeline-busan.sh와 동일 골격. 광주는 서해선(fill-only) 특례·지선(branch)
...
GEOM="$WORK/gwangju-geom.json"
TRACKS="$WORK/gwangju-tracks.json"
